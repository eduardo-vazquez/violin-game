...
navigator.mediaDevices.getUserMedia({ audio: true })
    .then(stream => {
        ...
        function detectPitch() {
            ...
            if (clarity > 0.95 && pitch) {
                ...
                text.textContent = `${Math.round(avgPitch)}`;
            } else {
                // Si no se detecta frecuencia clara, muestra '-'
                text.textContent = '-';
    ...
    .catch(err => {
        // Si ocurre un error (por ejemplo, el usuario niega el micrófono), lo muestra en consola
